...
The Foundation Cards system comes with a special default Type called **`File`**. This basic Type allows you to manage files, such as images, videos, PDFs and other media.
...
Creating a Type derived from `File` the same as creating any other derived Type (see steps for [creating derived Types](/ru/cards/types/creating-types/#creating-derived-types)).
...
Creating Cards from `File` Types is the same as creating Cards from any other Type (see our guide for [Creating Cards](/ru/cards/creating-cards)) — when you create a Card from a `File` Type, however, you'll see a designated area for uploading files.
...
The `File` Type is a powerful tool for managing files in your Foundation workspace. Here are some tips for getting the most out of it:
...
* **[Relations](/ru/cards/relations/relations-overview)** - Link your files to other Cards in your knowledge management system to establish connections between related resources.
...
* **Card attachments** - Attach files to any Card, regardless of its Type, by adding them directly to the Card’s interface (see [Creating Cards](/ru/cards/creating-cards) for an example). This is a great way to quickly attach a file that doesn't need its own dedicated Card.
