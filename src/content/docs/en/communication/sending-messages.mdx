...
Foundation offers many options for team members to communicate directly within the Foundation platform without the need for an external app. Foundation’s chat functionality allows team members to exchange messages in real time through direct messaging and shared channels. 

This guide covers how to exchange messages with your team using Chat. Chat is also integrated with the rest of your workspace, so documents, issues, pull requests and other workspace objects can be accessed alongside your other conversations. For more on how Chat integrates other workspace activity, see our guide on [activity tracking in chat](/foundation-docs/en/communication/chat-activity).

## Ways to chat

There are a few different ways to chat with team members on Foundation:
...
* **Other workspace objects** - From Chat, you can access all activity on documents, issues, files and more. Read more in our guide on [activity tracking in chat](/foundation-docs/en/communication/chat-activity).

**Icons and indicators:**
* A red dot beside the chat indicates an unread message.
* A green dot next to a user's avatar indicates that they are online and have their Foundation workspace open.
...
> **Note:** Two channels are created by the system when you create the workspace: `#general` and `#random`. In order to archive these system-generated channels, you must be a Spaces Admin. For more on permissions, including how to set Spaces Admins, see our guide on [roles](/foundation-docs/en/advanced-settings/roles).
...
Like everywhere else in your Foundation workspace, chat messages support markdown formatting, attachments, mentions, emojis and text templates. 
