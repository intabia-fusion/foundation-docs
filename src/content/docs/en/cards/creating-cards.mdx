...
To use Cards effectively in Foundation, it’s important to first understand their relationship with Types. 
...
If you haven't already, check out our guide on [Creating Types](/foundation-docs/en/cards/types/creating-types) — then return here to learn how to create Cards from the Types you've made.
...
Spaces for Cards act like any other space in Foundation, meaning you can leave, join and archive spaces, and global admin roles will apply. For more on managing permissions at the workspace and space level, see our guide on [Roles and Permissions](/foundation-docs/en/advanced-settings/roles).
...
2. **Type** - the Type that defines the properties of your Card (see [Creating Types](/foundation-docs/en/cards/types/creating-types))
...
6. **Children** - an area for adding child Cards (see [Parent-child Cards](/foundation-docs/en/cards/parent-child-cards))
7. **Attachments** - an area for uploading files
8. **Activity** - a log of all changes made to the Card and area for commenting

> **Note:** Cards created from the `File` Type will have an additional area above the rich text editor for uploading files. To learn more about managing files with the `File` Type, see [File Types](/foundation-docs/en/cards/types/file-types).

Beyond these basic elements, Cards can also include:

* **[Relations](/foundation-docs/en/cards/relations/relations-overview)** - connections to other Cards
* **[Tags](/foundation-docs/en/cards/tags/tags-overview)** - optional properties that can be applied to Cards of different types

Relations and Tags are set at the Type level. In other words, when you want to create a new Relation or Tag for a Card, you'll need to define this on the Type in Settings. To see how, check out these guides on [Defining Relations](/foundation-docs/en/cards/relations/defining-relations) and [Creating Tags](/foundation-docs/en/cards/tags/creating-tags).
...
From here, you can enter information in all of the attributes you [defined on your Type](/foundation-docs/en/cards/types/creating-types). Below is an example of a Card for an NPC character in our **game design system**:

<Image src={cardExampleNPC} alt="Example of an NPC Card" inferSize quality="max" />

Note that this `NPC` Card has several [Relations](/foundation-docs/en/cards/relations/relations-overview), defined on the `Character` Type from which it was derived:
 ...
 * [Relating Cards](/foundation-docs/en/cards/relations/relating-cards) - Define structured relationships between Cards.
* [File Types](/foundation-docs/en/cards/types/file-types) - Manage images, videos, PDFs, and other media within Foundation's default `File` Type.
* [Tags](/foundation-docs/en/cards/tags/tags-overview) - Apply additional attributes across different Types.

By understanding and structuring Cards effectively, you can build a powerful and flexible knowledge management system in Foundation.