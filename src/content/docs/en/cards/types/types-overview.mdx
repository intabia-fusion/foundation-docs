...
4. **`File`** - Digital files used in the game, such as images, sounds, textures and 3D models. This is a default system `File` Type (see [File Types](/en/cards/types/file-types)).

Each **Type** has a set of **properties** that define the specific details we want to capture for each piece of information.

We can also derive Types to define variations that have their own unique properties. To learn how to create Types and derived Types in practice, check out our next guide on [Creating Types](/en/cards/types/creating-types).