...
The Intabia Platform Cards system comes with a special default Type called **`File`**. This basic Type allows you to manage files, such as images, videos, PDFs and other media.
...
<Image src={blankFileCard} alt="File Card" inferSize />
...
Creating a Type derived from `File` the same as creating any other derived Type (see steps for [creating derived Types](/en/cards/types/creating-types/#creating-derived-types)).
...
<Image src={gameAssetTypes} alt="Types derived from Game Asset" inferSize />
<Image src={imageAsset} alt="Image asset properties" inferSize />
<Image src={audioAsset} alt="Audio asset properties" inferSize />

## Creating Cards from `File` Types

Creating Cards from `File` Types is the same as creating Cards from any other Type (see our guide for [Creating Cards](/en/cards/creating-cards)) — when you create a Card from a `File` Type, however, you'll see a designated area for uploading files.
...
<Image src={soundEffectCard} alt="Sound effect Card" inferSize />

> **WARNING:** Once uploaded, a file **cannot be deleted** from the Card.

## Tips for using the `File` Type

The `File` Type is a powerful tool for managing files in your Intabia Platform workspace. Here are some tips for getting the most out of it:
...
* **[Relations](/en/cards/relations/relations-overview)** - Link your files to other Cards in your knowledge management system to establish connections between related resources.
...
* **Card attachments** - Attach files to any Card, regardless of its Type, by adding them directly to the Card’s interface (see [Creating Cards](/en/cards/creating-cards) for an example). This is a great way to quickly attach a file that doesn't need its own dedicated Card.
