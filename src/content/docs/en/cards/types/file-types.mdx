...
Creating a Type derived from `File` the same as creating any other derived Type (see steps for [creating derived Types](/foundation-docs/en/cards/types/creating-types/#creating-derived-types)).
...
Creating Cards from `File` Types is the same as creating Cards from any other Type (see our guide for [Creating Cards](/foundation-docs/en/cards/creating-cards)) — when you create a Card from a `File` Type, however, you'll see a designated area for uploading files.
...
* **[Relations](/foundation-docs/en/cards/relations/relations-overview)** - Link your files to other Cards in your knowledge management system to establish connections between related resources.
...
* **Card attachments** - Attach files to any Card, regardless of its Type, by adding them directly to the Card’s interface (see [Creating Cards](/foundation-docs/en/cards/creating-cards) for an example). This is a great way to quickly attach a file that doesn't need its own dedicated Card.
